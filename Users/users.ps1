...
# Load the readMe contents into memory
...
foreach($admin in $sysAdmins) {
	# There is no need to remove the built in administrator account, or the main admin (you)
	if(($admin -eq "Administrator") -or ($admin -eq $mainAdmin)) {
		continue
	}
...
foreach($user in $sysUsers) {
	# There will never be a situation where the main admin, or default account needs to be removed
...
foreach ($account in $defaultUsers) {
	...
	if ((Get-LocalUser -Name $account).Enabled) {
		...
		$confirm = Read-Host -Prompt "The, $account, account is enabled. Would you like to disable? (Y/N)"
		if($confirm -eq "Y") {
